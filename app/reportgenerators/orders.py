...
def generate_orders():
    ...
    # Populate template
    pivot_df = company_df.pivot_table(
        index="Item Number", columns=["Invoice No.", "Store", "Date"], values="Quantity", fill_value=0
    )
    pivot_df = pivot_df.reindex(sorted(pivot_df.columns, key=lambda x: x[2]), axis=1)  # Sort cols by date asc

    template_item_codes = set(
        sheet[f'{lookup[code]["template_code"]["col"]}{row}'].value
        for row in range(lookup[code]["template_code"]["start_row"], lookup[code]["template_code"]["end_row"] + 1)
    )
    ...
    for idx, new_item_code in enumerate(new_item_codes, start=lookup[code]["template_code"]["end_row"] + 1):
        ...
        sheet[f'{lookup[code]["template_item"]["col"]}{idx}'] = company_df[company_df["Item Number"] == new_item_code][
            "Description"
        ].values[0]
        sheet[f'{lookup[code]["template_price"]["col"]}{idx}'] = get_price(
            company_df[company_df["Item Number"] == new_item_code]["Price"].values[0]
        )

    # -- Write dates invoice, row, stores rows
    for col_num, (invoice, store, date) in enumerate(
        pivot_df.columns, start=lookup[code]["template_date"]["start_row"]
    ):
    ...
    # -- Fill in quantities
    for row_num, code_row in enumerate(
        range(lookup[code]["template_code"]["start_row"], lookup[code]["template_extra"]["end_row"] + 1),
        start=lookup[code]["template_code"]["start_row"],
    ):
        ...
        if item_code_val in pivot_df.index:
            for col_num, (invoice, store, date) in enumerate(
                ...
                if item_code_val == fhc_code:
                    ...
                    price_entry = company_df[
                        (company_df["Item Number"] == fhc_code)
                        & (company_df["Store"] == store)
